...
static int read_fallback_colors(
    const char *colors_path,
    char *bg, size_t bg_size,
    char *fg, size_t fg_size
) {
    struct json_object *root = json_object_from_file(colors_path);
    if (!root) return -1;
    ...
    if (json_object_object_get_ex(root, "special", &special) &&
        ...
        struct json_object *background = NULL;
        struct json_object *foreground = NULL;
        if (json_object_object_get_ex(special, "background", &background) &&
            json_object_is_type(background, json_type_string)) {
            bg_value = json_object_get_string(background);
        }
        if (json_object_object_get_ex(special, "foreground", &foreground) &&
            json_object_is_type(foreground, json_type_string)) {
            fg_value = json_object_get_string(foreground);
        }
    }
    if (!bg_value || !fg_value) {
        if (json_object_object_get_ex(root, "colors", &colors) &&
            json_object_is_type(colors, json_type_object)) {
            struct json_object *color0 = NULL;
            struct json_object *color7 = NULL;
            if (json_object_object_get_ex(colors, "color0", &color0) &&
                json_object_is_type(color0, json_type_string)) {
                bg_value = json_object_get_string(color0);
            }
            if (json_object_object_get_ex(colors, "color7", &color7) &&
                json_object_is_type(color7, json_type_string)) {
                fg_value = json_object_get_string(color7);
            }
        }
    }
    json_object_put(root);
    if (!bg_value || !fg_value) return -1;

    if (bg_value[0] == '#') bg_value++;
    if (fg_value[0] == '#') fg_value++;
    if (snprintf(bg, bg_size, "%s", bg_value) >= (int)bg_size ||
        snprintf(fg, fg_size, "%s", fg_value) >= (int)fg_size) {
        return -1;
    }
    return 0;
}
...
int css_write_boot_fallback(const char *colors_path) {
    ...
    if (read_fallback_colors(colors_path, bg, sizeof(bg), fg, sizeof(fg)) != 0) {
        return 0; /* no palette yet — nothing to fall back to */
    }
    ...
    const int output_length = snprintf(
        ...
        "html:not([data-darkreader-scheme]) body {\n"
